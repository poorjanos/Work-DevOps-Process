...
query_timesheet <- readQuery(here::here("SQL", "get_workhours_per_issue.sql"))
...
# Genarate nested df
# Gen nested tables with aggregated stats in nested tables
by_class_appgroup_app <- t_isd_eventlog %>%
  group_by(CLASSIFICATION, APPGROUP, APPLICATION) %>%
  nest() %>%
  mutate(
    CASE_NUMBER = map(data, ~length(unique(.$CASE))),
    EVENT_NUMBER = map(data, ~length(unique(.$ACTIVITY))),
...
<!-- ## 4.1. Absolute Frequency of INC Ticket Types between 2017-06 and 2019-05 -->

```{r, fig.width=10, include=FALSE}
by_class_appgroup_app %>%
  # Transform data
  filter(CLASSIFICATION == "Hibabejelentés (INC)") %>%
  select(APPLICATION, APPGROUP, CASE_NUMBER) %>% 
  unnest() %>%
  mutate(APPLICATION = forcats::fct_lump(factor(APPLICATION), w = CASE_NUMBER, n = 20)) %>% 
  replace_na(list(APPGROUP = "NA")) %>%
  mutate(
    APPGROUP =
      forcats::fct_relevel(
  ...
  group_by(APPGROUP, APPLICATION) %>% 
  summarize(CASE_NUMBER = sum(CASE_NUMBER)) %>% 
  ungroup() %>% 
  arrange(APPGROUP, APPLICATION) %>% 
  # Plot
  ggplot(aes(
    x = APPGROUP,
    y = CASE_NUMBER,
  ...
  labs(
    x = "Application Group",
    y = "Absolute Frequency (count)",
    title = "Absolute Frequency of INC Ticket Types (2 years span)",
    fill = "Ticket Type"
  )
```


# 4. Timesheet Missing Value Patterns


```{r}
sum(is.na(t_timesheet$HOURS))
```
