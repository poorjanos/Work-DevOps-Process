...
Timesheet hours missing for 90% of tickets

```{r, include = FALSE}
sum(is.na(t_timesheet$HOURS)) / length(t_timesheet$HOURS)
```
...
t_timesheet %>%
  # Transform data
  group_by(APPGROUP) %>%
  summarize(TIMESHEET_MISSING = sum(is.na(HOURS)) / length(HOURS)) %>%
  ungroup() %>%
  replace_na(list(APPGROUP = "NA")) %>%
  filter(APPGROUP != "NA") %>%
  mutate(APPGROUP = forcats::fct_rev(forcats::fct_recode(APPGROUP, Közös = "Z_Közös"))) %>%
  ...
  geom_bar(stat = "identity") +
  scale_y_continuous(labels = percent) +
  geom_text(aes(label = paste0(round(TIMESHEET_MISSING, 2) * 100, "%")), size = 4, hjust = 0) +
  theme(
  ...
  labs(
    x = "Application Group",
    y = "Ratio of Tickets with Missing Timesheet Data",
    title = "Ratio of Missing Timesheets"
  )
```
