...
## 2.1. Total Numner of Cases by Applicaton Group Between 2017-06 and 2019-05
...
by_case_type %>%
  ...
  labs(
    x = "Application Group",
    y = "# of Cases",
    title = "Number of Cases (2 years span)"
  )
```


## 2.2. Traces by Applicaton Group (Process Complexity Based on Trace Variation)

Average trace coverage gives an insight to the complexity of the underlying process. The more traces needed to cover a 100 cases the higher the variation of traces, thus the more complex the process.
...
## 2.3 Trace Coverage by Application Group
...
by_case_type %>% select(CLASSIFICATION, APPGROUP, THROUGH_TIME_BY_CASE) %>%
  unnest() %>%
  replace_na(list(APPGROUP = "NA")) %>%
  mutate(APPGROUP = forcats::fct_rev(
    forcats::fct_relevel(
...
by_case_type %>% select(CLASSIFICATION, APPGROUP, TRACE_LENGTH_BY_CASE) %>%
  unnest() %>%
  replace_na(list(APPGROUP = "NA")) %>%
  mutate(APPGROUP = forcats::fct_rev(
    forcats::fct_relevel(
